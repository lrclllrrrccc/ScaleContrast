...
class DAN(tf.keras.Model):
    ...
    def __init__(self,
        ...
        if not transform:
            self.transform = tf.keras.Sequential([tf.keras.layers.Dense(units, activation=tf.gelu)] * 2)
        elif isinstance(transform, Sequence):
            self.transform = tf.keras.Sequential(tf.keras.layers)
        else:
...
class CantripModel(object):
    ...
    def _add_seq_rnn(self, cell_type: str):
        ...
        with tf.variable_scope('sequence'):
            ...
            self.seq_final_output = tf.keras.layers.rnn_layer(self.cell_fn, self.num_hidden, self.x, self.seq_lengths)
