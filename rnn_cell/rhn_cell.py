...
linear = ran_cell._linear

class RHNCell(tf.keras.layers.Layer):
    ...
    def zero_state(self, batch_size, dtype):
        ...
        input_shape = tf.as_shape(batch_size + [tf.as_dimension(self.input_size)])
        if self._keep_i < 1.0:
            noise_i = tf.random.uniform(shape=input_shape, dtype=dtype) / self._keep_i
        else:
            noise_i = tf.ones(shape=input_shape, dtype=dtype)

        state_shape = tf.as_shape(batch_size + [tf.as_dimension(self._num_units)])
        if self._keep_h < 1.0:
