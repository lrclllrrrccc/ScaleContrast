...
# #     """
# #     Creates an RNN encoder with the given number of hidden layers. If
# #     num_hidden is iterable, a multi-layer rnn cell will be created using each number of hidden units.
# #
# #     :param num_hidden: number of hidden (memory) units to use; num_hidden is iterable for a multi-layer RNN
# #     :param cell_fn: rnn_cell constructor to use (e.g., RANCell, LSTMCell, etc.)
# #     :return: rnn_encoder function
# #     """
# #
# #     def _rnn_encoder(model):
...
# #
# #         # Apply RNN to all documents in all batches
# #         # Check if num_hidden is iterable and create a multi-layer RNN
# #         if isinstance(num_hidden, (list, tuple)):
# #             rnn_layers = [cell_fn(units) for units in num_hidden]
# #             rnn_layer = tf.keras.layers.StackedRNNCells(rnn_layers)  # 使用堆栈的 RNN 单元
# #         else:
# #             rnn_layer = cell_fn(num_hidden)
# #
# #         rnn = tf.keras.layers.RNN(rnn_layer, return_sequences=True)
# #
# #         # Apply the RNN layer
# #         flattened_snapshot_encodings = rnn(flattened_embedded_obs)
# #
# #         # Reshape back to (batch x seq_len x encoding_size)
# #         return tf.reshape(flattened_snapshot_encodings,
# #                           [model.batch_size, model.max_seq_len, flattened_snapshot_encodings.shape[-1]],
...
# #     Creates a CNN encoder with the given number of windows, kernels, and dropout
# #     :param windows: number of consecutive observations to consider; defaults to [3, 4, 5]
# #     :param kernels: number of convolutional kernels; defaults to 1000
# #     :param dropout: dropout probability; defaults to 0.0 (no dropout)
# #     :return: cnn_encoder function
# #     """
# #     if windows is None:
...
# #         for n in windows:
# #             if dropout > 0:
# #                 flattened_embedded_obs = tf.keras.layers.Dropout(rate=dropout)(flattened_embedded_obs, training=model.training)
# #
# #             conv_layer = tf.keras.layers.Conv1D(filters=kernels,
# #                                                  kernel_size=n,
# #                                                  activation=tf.nn.leaky_relu,
# #                                                  name="conv_%dgram" % n)(flattened_embedded_obs)
# #             pool_layer = tf.keras.layers.MaxPooling1D(pool_size=1,
...
# # def dense_encoder(model):
# #     """
# #     Represents documents as an embedded bag of clinical observations. Specifically, returns an embedded V-length
# #     binary vector encoding all clinical observations included in a snapshot.
# #
# #     :param model: CANTRIP model
# #     :type model: modeling.CANTRIPModel
# #     :return: clinical snapshot encoding
# #     """
# #     # Use the CPU cause things are about to be weird (i.e., too big to fit in GPU memory)
# #     with tf.device("/cpu:0"):
# #         # Add bag-of-observation vector transformations to the model
# #         bags = get_bag_vectors(model)
# #
...
# #
# #         # Sparse to dense projection
# #         flat_doc_embeddings = tf.sparse.sparse_dense_matmul(flat_emb_bags, embedded_observations,
# #                                                             name='flat_doc_embeddings')
# #
...
#     :return: clinical snapshot encoding
#     """
#     with tf.name_scope('bow_encoder'):
...
# #     # Use the CPU cause everything will be vocabulary-length
# #     with tf.device("/cpu:0"):
# #         # Get the bag of vectors and convert to dense representation
# #         return tf.sparse.to_dense(get_bag_vectors(model))
...
#     }.get(activation, KeyError(f'Unsupported activation function: {activation}'))
#
...
#         :return: clinical snapshot encoding
#         """
#         # Remove 'training=model.training' from the embedding_layer call
#         embedded_observations = layers.embedding_layer(
#             model.observations,
#             model.vocabulary_size,
#             model.embedding_size,
#             model.vocab_dropout
...
#             avg_layer = tf.keras.layers.Dense(num_hidden, activation=activation_fn)(avg_layer)
#
#         # Final output of the model
#         output = tf.keras.layers.Dense(model.embedding_size, activation=activation_fn)(avg_layer)
#
...
# #         :return:
# #         """
# #         with tf.name_scope('dan_encoder'):
# #             embedded_observations = layers.embedding_layer(model.observations, model.vocabulary_size,
...
#         with tf.name_scope("residual_unit%d" % i):
...
# # def rmlp_encoder(activation='gelu', num_layers=10, num_hidden=2048):
# #     """Creates a residual MLP encoder."""
# #
# #     # Define activation function
# #     activation_fn = {
# #         'gelu': layers.gelu,
...
# #
# #     def _rmlp_encoder(model):
# #         """Represents documents as a residual MLP encoding."""
# #         # Convert batch x seq_len x doc_len tensor of obs IDs to bag-of-observation vectors
# #         bags = get_bag_vectors(model)
# #         flat_bags = tf.sparse.reshape(bags, [model.batch_size * model.max_seq_len, model.vocabulary_size])
# #         x = SparseDenseLayer(units=num_hidden, activation=None)(flat_bags)
# #
...
import tensorflow as tf
import layers  # 假设自定义的 layers 模块存在
import rnn_cell  # 假设自定义的 rnn_cell 模块存在


def rnn_encoder(num_hidden, cell_fn=rnn_cell.RANCell):
    """
    Creates an RNN encoder with the given number of hidden layers.
    """
    def _rnn_encoder(model):
        with tf.name_scope('rnn_encoder'):
            # Embed clinical observations
...
def cnn_encoder(windows=None, kernels=1000, dropout=0.):
    ...
    #                                                 kernel_size=n,
    #                                                 activation=tf.nn.leaky_relu,
    #                                                 name=f"conv_{n}gram")(flattened_embedded_obs)
    #             pool_layer = tf.keras.layers.MaxPooling1D(pool_size=1,
    #                                                       strides=model.max_snapshot_size - n + 1,
    #                                                       name=f"maxpool_{n}gram")(conv_layer)
    #             outputs.append(pool_layer)
    #
    #         # Concatenate pooled outputs
    #         output = tf.concat(outputs, axis=-1)
    ...
    #
    # return _cnn_encoder
    def _cnn_encoder(model: tf.keras.Model, windows: List[int], kernels: int, dropout: float) -> Callable[[tf.keras.Model, tf.Tensor, bool], tf.Tensor]:
        def cnn_encoder(inputs: tf.Tensor, training: bool = False) -> tf.Tensor:
            with tf.name_scope('cnn_encoder'):
                # Reshape to (batch * seq_len) x snapshot_size x embedding
                flattened_embedded_obs = tf.reshape(inputs,
                                                    [model.batch_size * model.max_seq_len,
                                                     model.max_snapshot_size,
                                                     model.embedding_size])
                ...
                for n in windows:
                    if dropout > 0:
                        flattened_embedded_obs = tf.keras.layers.Dropout(rate=dropout)(flattened_embedded_obs,
                                                                                       training=training)
                    conv_layer = tf.keras.layers.Conv1D(filters=kernels,
                                                        kernel_size=n,
                                                        activation=tf.nn.leaky_relu,
...
def get_bag_vectors(model):
    """
    Represents snapshots as a bag of clinical observations.
    """
    with tf.name_scope('bag_vectors'):
        # 1. Evaluate which entries in model.observations are non-zero
        mask = tf.not_equal(model.observations, 0)
        where = tf.where(mask)
        ...
        # The dense shape will be the same as model.observations, but using the entire vocabulary as the final dimension
        dense_shape = model.observations.shape.as_list()
        dense_shape[2] = model.vocabulary_size

        # Store as a sparse tensor
        st = tf.sparse.SparseTensor(indices=indices, values=ones, dense_shape=dense_shape)
        return tf.sparse.reorder(st)


# Additional encoders (dense_encoder, bag_encoder, dan_encoder, rmlp_encoder, vhn_encoder) can be adapted similarly.
# Due to space constraints, only key methods (rnn_encoder, cnn_encoder, get_bag_vectors) are shown here.


### SparseDenseLayer Implementation
class SparseDenseLayer(tf.keras.layers.Layer):
    def __init__(self, units, activation=None, use_bias=True, **kwargs):
        super(SparseDenseLayer, self).__init__(**kwargs)
        self.units = units
        self.activation = tf.keras.activations.get(activation)
        self.use_bias = use_bias
        self.kernel = self.add_weight(shape=(None, units), initializer='glorot_uniform', trainable=True)
        if self.use_bias:
            self.bias = self.add_weight(shape=(units,), initializer='zeros', trainable=True)

    def call(self, inputs):
        if not isinstance(inputs, tf.sparse.SparseTensor):
