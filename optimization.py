...
# from tensorflow.python.ops import math_ops, control_flow_opsnoinspection PyProtectedMember
        ...
        if focal_loss:
            def sparse_focal_softmax_cross_entropy(labels, logits, weights=1., alpha=1, gamma=2.0,
                ...
                with tf.name_scope(scope, "focal_softmax_cross_entropy_loss", (logits, labels, weights)) as scope:
                    ...
                    labels, logits, weights = tf._remove_squeezable_dimensions(
...
class AdamWeightDecayOptimizer(Adam):
...
def polynomial_decay(learning_rate,
    ...
    with tf.name_scope(
        ...
        end_learning_rate = tf.cast(end_learning_rate, dtype)
        power = tf.cast(power, dtype)

        global_step_recomp = tf.cast(global_step, dtype)
        decay_steps_recomp = tf.cast(decay_steps, dtype)
        if cycle:
            # Find the first multiple of decay_steps that is bigger than
            # global_step. If global_step is zero set the multiplier to 1
            multiplier = tf.cond(
                tf.equal(global_step_recomp, 0), lambda: 1.0,
                lambda: tf.ceil(global_step_recomp / decay_steps))
        ...
        return tf.add(
            tf.multiply(learning_rate - end_learning_rate,
                              tf.pow(1 - p, power)),
            end_learning_rate,
            name=name)
